...
* ##### Trigger Logic:
After Insert: Whenever a new row is inserted into the borrowing_transactions table, this trigger checks the action field:
If the action is 'Borrowed', the availability status of the corresponding book is set to 'not_available'.
If the action is 'Returned', the availability status is set to 'available'.
### Relationships
