...
It is a To-do list program which can be completely run from terminal. It lets user create a 'To-do list' which contains a list of tasks. It also give users option to view, add, delete task and aslo mark task as done. When exiting the program  I made it very easy to use. One can simply use the program by reading only what is being shown on screen. User friendly feedback messages are displayed making it easy for the user to understand the working of program. Feedback messages and Instructions are colourized and boxed to making it easy to read.
